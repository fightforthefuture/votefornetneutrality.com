...
// SETUP:
//
// In store/index.js requires new state variables:
//
// ```
...
// }
// ```
// and the mutations: `setMapZoom` and `setMapCurrentPin`
//
// In package.json dependencies requires: `"leaflet": "^1.3.1"`
//
// config.json requires: `mapboxToken` to be defined and
//
...
export default {
  ...
  computed: {
    mapboxToken () { return this.$store.state.mapboxToken },
    zoomLevel: {
      get() {
  ...
  methods: {
    ...
    addMarker(event) {
      ...
      html += `<div class="address"><address>${event.address}</address></div><a class="btn btn-hollow btn-sml btn-block" href="${event.url}" target="_blank">${event.category === 'facebook_group' ? 'Join Facebook Group' : 'Info & RSVP'}</a>`
